...
import { useDashboardData, useProfile } from "./use-dashboard-data";
...
describe("useDashboardData", () => {
  ...
  it("returns birds and logs when user has flock memberships", async () => {
    const birds = [
    ...
    expect(result.current.data?.birds).toEqual(birds);
    expect(result.current.data?.logs).toEqual(logs);
  });

  it("returns empty arrays when user has no flock memberships", async () => {
    mockClient.createClient.mockReturnValue(
      createMockQueryBuilder(
        {
          flock_members: [],
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data?.birds).toEqual([]);
    expect(result.current.data?.logs).toEqual([]);
  });

  it("returns birds with empty logs when no logs exist", async () => {
    const birds = [
      { id: "bird-1", name: "Tweety", species: "Budgie", current_weight: 30, target_weight: 32, avatar_color: "blue", status: "active", timezone: "UTC", sort_order: 0 },
    ];
